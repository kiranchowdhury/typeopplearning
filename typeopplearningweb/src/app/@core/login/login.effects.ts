...
export class LoginEffects {
    ...
    loginSuccess(): Observable<Action> {
        return this.actions$
            ...
            .pipe(
                tap((action: ActionSignInSuccess) => this.userContext.loadUserContext()),
                tap((action: ActionSignInSuccess) => action.payload.role === 'ADMIN'?
                                        this.router.navigate(['/pages/customers'])
                                        : this.router.navigate(['/pages/howto']))
            )
    }
